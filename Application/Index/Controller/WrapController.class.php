...
class WrapController extends Controller {
    ...
    public function __construct() {
        ...
        if ($this->allowAccess() || $this->checkLogin()) {
            ...
            if (IS_AJAX) {
                $this->ajaxError("<a href='" . U('Index/Admin/login') . "'>请重新登录</a>");
            }
            $this->error('请重新登录', U('Index/Admin/login'));
        }
    }
    ...
    private function checkRightRule() {
        ...
        if (!$auth->check($rule_name, $uid)) {
            if (IS_AJAX) {
                ...
                $this->error('目前不具备该功能权限', U('Index/Admin/index'));
            }
        }
    }
    ...
    private function checkAccessIp() {
        ...
        $this->error($remoteIp . '不在IP范围内', U('Index/Admin/login'));
    }
...
//        }
        ...
        $userModel = new \Index\Model\AuthuserModel();
