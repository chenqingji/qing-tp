...
class CustomController extends ServiceController {
    ...
    public function index() {
        $menuCrumbs = array(
            ...
            "second" => array("menuName" => "在仓库定制品列表", "url" => "/Warehouse/Custom/index"),
        );
        $this->assign("menuCrumbs", $menuCrumbs);

        $customModel = new \Warehouse\Model\CustomModel();
...
//        $this->success("已分配了仓位：" . $location, U("putin"), 5);
     ...
     * @param \Warehouse\Model\CustomModel $customModel
     * @return boolean
     */
...
//        $this->error($msg, U("putin"), 3);
    ...
    private function allotOldLocation($data, $curProductInfo, $oneEmptyRecord, $customModel) {
        ...
        $res2 = $inRecordModel->baseAdd(array(
            ...
            "op_time" => time(),
            "type" => \Warehouse\Model\InrecordModel::TYPE_CUSTOM,
...
//        $nextLocation  = $nextShelf.$nextLayer.$nextCol;
    ...
    public function viewProcess() {
        ...
        if (!isset($_POST['operator'])) {
            $menuCrumbs = array(
                "first" => $this->_menuCrumbsFirst,
                "second" => array("menuName" => "定制品进度查询", "url" => "/Warehouse/Custom/viewprocess"),
            );
            $this->assign("menuCrumbs", $menuCrumbs);
                ...
                if ($productList) {
                    if ($productList['process'] == 0) {
                        ...
                        if ($productList['process'] >= 1) {
                            $list['process'] = "已入库";
                            $inRecordModel = new \Warehouse\Model\InrecordModel();
                            $record = $inRecordModel->baseFind(array('category_id' => $productId, "type" => \Warehouse\Model\InrecordModel::TYPE_CUSTOM),"*",array("id"=>"desc"));
                            if ($record) {
                        ...
                        if ($productList['process'] >= 2) {
                            $list['process'] = "已拣货";
                            $pickModel = new \Warehouse\Model\PicklistModel();
...
//                                    $list['outTime'] = $one['date'];
                                ...
                                $outRecord = $outRecordModel->baseFind(array("category_id" => $productId, "type" => array("eq", \Warehouse\Model\OutrecordModel::TYPE_CUSTOM)),"*",array("id"=>"desc"));
                                if ($outRecord) {
