...
class Common_util_pub
{
     ...
     * 	作用：生成签名
     */
    public function getSignEx($Obj, $key)
    {
        foreach ($Obj as $k => $v)
        {
            $Parameters[$k] = $v;
     ...
     * 	作用：array转xml
     */
    function arrayToXml($arr, $numeNoConvert = true)
    {
        $xml = "<xml>";
        foreach ($arr as $key=>$val)
        {
            if (is_numeric($val) && $numeNoConvert)
            {
                $xml.="<".$key.">".$val."</".$key.">";
...
class Wxpay_server_pub extends Common_util_pub
{
    ...
    function checkSignEx($key)
    {
        $tmpData = $this->data;
        unset($tmpData['sign']);
        $sign = $this->getSignEx($tmpData, $key);//本地签名
        if ($this->data['sign'] == $sign) {
            return TRUE;
        }
        return FALSE;
 ...
 * 微信红包
 */
class Wx_RedPack extends Common_util_pub
{
    public $parameters = array();//请求参数，类型为关联数组
    public $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack"; //接口链接
    public $curl_timeout = WxPayConf_pub::CURL_TIMEOUT;//curl超时时间
    public $response; //微信返回的响应
    ...
    {
        $this->parameters[$this->trimString($key)] = $this->trimString($value);
    }
    ...
    function createXml()
    {
        ...
        $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
        $this->parameters["sign"] = $this->getSign($this->parameters);//签名
        return  $this->arrayToXml($this->parameters,false);
    }

    function post() {
        $ch = curl_init();
        $CertFile = realpath(VENDOR_PATH .WxPayConf_pub::SSLCERT_PATH);
        $keyFile = realpath(VENDOR_PATH .WxPayConf_pub::SSLKEY_PATH);
        $caFile = realpath(VENDOR_PATH .WxPayConf_pub::CA_PATH);
