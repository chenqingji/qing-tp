...
class Pages{
    ...
    public function __construct($totalRows, $listRows=20, $parameter = array()) {
        ...
        $this->parameter  = empty($parameter) ? $empty_arr : $parameter;
    ...
    private function factoryUrl($root , $param){
        $param_str = '';
        foreach ($param as $k => $v) {
            $param_str .= $k.'/'.$v.'/';
        }
        $param_str = rtrim($param_str,'/');
        return $root.'/'.$param_str;
    }
     ...
     * @return string
     */
    public function show( $method = 'index' ) {
        if(0 == $this->totalRows) return '';

        /* 生成URL */
        $this->parameter[$this->p] = urlencode('[PAGE]');

        $this->url = $this->factoryUrl( $_SERVER['site_root'].'/'.$method , $this->parameter );
